...
// Auth is used to authenticate cleints. It can generate a token for a
...
// New creates a *Authenticator for use.
func New(algorithm string, lookup PublicKeyLookup, keys Keys) (*Auth, error) {
	...
	keyFunc := func(t *jwt.Token) (interface{}, error) {
		kid, ok := t.Header["kid"]
		if !ok {
			return nil, errors.New("missing key id (kid) in toke header")
		}
		kidID, ok := kid.(string)
		if !ok {
	...
	// validated to avid a critical vulnerability:
