...
func startContainer(t *testing.T, image string, port string, args ...string) *Container {
	...
	if err := cmd.Run(); err != nil {
		t.Fatalf("coul not start container %s: %v", image, err)
	}
...
func extractIPPort(t *testing.T, doc []map[string]interface{}, port string) (string, string) {
	...
	if !exists {
		t.Fatalf("could not get network ports settings")
	}
	tcp, exists := ports.(map[string]interface{})[port+"/tcp"]
	if !exists {
		t.Fatalf("could not get network ports/tcp settings")
	}
	list, exists := tcp.([]interface{})
	if !exists {
		t.Fatalf("could not get network ports/tcp list settings")
	}
	if len(list) != 1 {
		t.Fatalf("could not get network ports/tcp list settings")
	}
	...
	if !exists {
		t.Fatalf("could not get network ports/tcp list data")
	}
