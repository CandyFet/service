...
import (
	...
	"github.com/CandyFet/service/business/data/schema"
...
// NewUnit create a test database inside a Docker container. It creates the
// required table structure but the database is otherwise empty. It returns
// the database to use as wel las a function to call at the end of the test.
func NewUnit(t *testing.T) (*log.Logger, *sqlx.DB, func()) {
	...
	// teardown is the function that should be invoked whe the calles is done
	// with the database
	teardown := func() {
		t.Helper()
		db.Close()
...
// Context return an app level context for testing.
...
// NewIntegration create a database, seeds it, constructs an authenticator.
func NewIntegration(t *testing.T) *Test {
	...
	kidID := "d8420866-1615-43d3-9a25-928120080b8f"
...
// Token geneates an authenricated token for a user.
